...
As part of this project, we developed a model to identify clients who may require a timely reminder about their available funding. Each client holds a sum of funds in their account, and it is important to ensure they are aware of opportunities to use this funding before it remains unspent. The model aims to flag clients who are most in need of outreach, based on their funding patterns and service usage history.

To ensure data privacy and comply with security protocols, all personally identifiable information and original client data were removed. Instead, we created a representative mock dataset that captures the structure and behavioural patterns of the original data, enabling safe model development and evaluation without compromising confidentiality. 
